...
This map shows that the south eastern part of the state tends to have lower registration rates. 
...
combined_df %>% 
  ggplot(aes(x = opo, y = percent_enrolled)) +
    ...
    labs(x = "OPO", 
         y = "Percent of Eligible Population Enrolled",
         title = "Distribution of Percent Enrollment by OPO") +
   theme(axis.text.x = element_text(angle = 90, hjust = 1, size = rel(1)))
```

An organ procurement organization (OPO) is a non-profit organization that is responsible for the evaluation and procurement of deceased-donor organs for organ transplantation. There are 4 OPOs in the New York State, each with its own designated service area. From the plot above, we found that the distribution of registration rates in counties designated to the 4 OPOs are different. Obviously, the New York Organ Donor Network has the lowest distribution of Percent Enrollment, and will be used as the reference group in regression analysis.
...
b = regsubsets(percent_enrolled ~ ., data = regression_df, force.in = NULL)
summary(b) 
```

The first model is: percent_enrolled ~ opo + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15 + percent_male_2015 + percent_asian_2015 + percent_medicare_enrollment_2015.
```{r}
fit_1 = lm(percent_enrolled ~ opo + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15 + percent_male_2015 + percent_asian_2015 + percent_medicare_enrollment_2015, regression_df)
fit_1 %>% broom::glance()
fit_1 %>% broom::tidy()
```
...
```{r}
fit_2 =   lm(percent_enrolled ~ opo + percent_medicare_enrollment_2015 + percent_male_2015 + percent_white_2015 + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15, data = regression_df) 
fit_2 %>% broom::glance()
fit_2 %>% broom::tidy()
```
...
bootstrap_df = 
  ...
  mutate(
    model_1 = purrr::map(strap, ~lm(percent_enrolled ~ opo + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15 + percent_male_2015 + percent_asian_2015 + percent_medicare_enrollment_2015, data = .x)),
    model_2 = purrr::map(strap, ~lm(percent_enrolled ~ opo + percent_medicare_enrollment_2015 + percent_male_2015 + percent_white_2015 + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15, data = .x)),
    model_3 = purrr::map(strap, ~lm(percent_enrolled ~ opo + percent_medicare_enrollment_2015 + percent_male_2015 + percent_white_2015*percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15, data = .x))) 
...
cv_df = 
  cv_df %>% 
  mutate(
    model_1 = purrr::map(train, ~lm(percent_enrolled ~ opo + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15 + percent_male_2015 + percent_asian_2015 + percent_medicare_enrollment_2015, data = .x)),
    model_2 = purrr::map(train, ~lm(percent_enrolled ~ opo + percent_medicare_enrollment_2015 + percent_male_2015 + percent_white_2015 + percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15, data = .x)),
    model_3 = purrr::map(train, ~lm(percent_enrolled ~ opo + percent_medicare_enrollment_2015 + percent_male_2015 + percent_white_2015*percent_persons_25_w_hs_diploma_2011_15 + percent_persons_25_w_4_yrs_college_2011_15, data = .x))) %>% 
...
Many of the smaller counties in NYS already have the enrollment registration rates seen in surrounding states. However, the larger counties in the New York City area have rates that are ________ percentage points lower than those of surrounding states. It is great that people in small counties are willing to donate organs, but NY needs to target larger counties with smaller rates to take advantage of the number of potential organs these counties represent.   
